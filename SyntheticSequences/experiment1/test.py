...
for n_sequences in n_sequences_list:
    ...
    for i in range(0, n_experiment):
        ...
        if (final_k_results['k'] == clusters):
            ...
            computed_indexes = clustereval.calculate_external(partition_generated, partition_found)
            for key, value in final_statistics_external.items():
                final_statistics_external[key].append(
                    computed_indexes[list(final_statistics_external.keys()).index(key)])
...
# plt.show()
if (len(gap_values) == 11):
    title = 'experiment1_synthetic_nclusters_' + str(clusters) + '_rates_' + str(rates) + '_' + str(
        method) + '_positivegap' + '_percentage.png'
else:
    title = 'experiment1_synthetic_nclusters_' + str(clusters) + '_rates_' + str(rates) + '_' + str(
        method) + '_' + '_percentage.png'
...
for n_sequences in final_avgs_statistics.keys():
    ...
    for index in final_statistics_external.keys():
        table_avgs.append([final_avgs_statistics[n_sequences][index]])
        table_stds.append([final_stds_statistics[n_sequences][index]])
        table_medians.append([final_medians_statistics[n_sequences][index]])

if (len(gap_values) == 11):
    file_name = 'experiment1_synthetic_' + 'nclusters_' + str(clusters) + '_rates_' + str(rates) + '_' + str(
        method) + '_positivegap' + '_statistics' + '.txt'
else:
    file_name = 'experiment1_synthetic_' + 'nclusters_' + str(clusters) + '_rates_' + str(rates) + '_' + str(
        method) + '_' + 'statistics' + '.txt'

f = open(directory + file_name, 'w+')
f.write('Averages \n')
f.write(tabulate(table_avgs,
                 headers=['Number of sequences/cluster', 'R', 'AR', 'FM', 'J', 'AW', 'VD', 'H', 'H\'', 'F', 'VI',
                          'MS']))
f.write(' \n Standard Deviations \n')
f.write(tabulate(table_stds,
                 headers=['Number of sequences/cluster', 'R', 'AR', 'FM', 'J', 'AW', 'VD', 'H', 'H\'', 'F', 'VI',
                          'MS']))
f.write(' \n Medians \n')
f.write(tabulate(table_medians,
                 headers=['Number of sequences/cluster', 'R', 'AR', 'FM', 'J', 'AW', 'VD', 'H', 'H\'', 'F', 'VI',
                          'MS']))
f.write('\n Ratio list: \n ')
